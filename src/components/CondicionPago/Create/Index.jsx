...
export const CondicionPagoCreate = () => {
	...
	const handlerFormSubmit = (e) => {
		...
		toast
			.promise(CondicionPagoService.Create(json), {
				...
				error: {
					render({ data }) {
						return ERROR_CASES[data?.response?.data?.error] || "Error al crear la condición de pago.";
					},
				},
			})
			.then((x) => {
			...
			<Form submitButtonClass="btn-success" submitButtonName="Guardar" handlerSubmit={handlerFormSubmit}>
				<div className="row mt-4">
					...
					<div className="col-md-4">
						...
						/>
					    <span className="text-danger">{formData.cantidad_dias_error}</span>
					</div>
				</div>
				<div className="row mt-4">
