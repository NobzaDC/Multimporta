...
export const PuertosCreate = () => {
	...
	const handlerFormSubmit = (e) => {
		e.preventDefault();
		setFormData((last) => ({ ...last, codigo_error: "", nombre_error: "", tiempo_error: "" }));
		...
		toast
			.promise(PuertosService.Create(json), {
				...
				error: {
					render({ data }) {
						return ERROR_CASES[data?.response?.data?.error] || "Error al crear el puerto.";
					},
				},
			})
			.then((x) => {
	...
	return (
		<Section>
			<NavigationTitle name="Nuevo puerto" path={getServerPath(PUERTO_PATH.index)} />
